...
class ExtractedClause(BaseModel):
    ...
    section_ref: str = Field("", description="Section reference in the original document, e.g. '4.2.1'")
    text: str = Field(..., description="Full text of the clause")
    parties: list[str] = Field(default_factory=list, description="Parties referenced in this clause")
...
class SLAEntry(BaseModel):
    ...
    penalty_on_breach: Optional[str] = Field(None, description="Penalty description if SLA is breached")
...
class Obligation(BaseModel):
    ...
    recurrence: Optional[str] = Field(None, description="Recurrence pattern, e.g. 'monthly', 'quarterly'")
...
class PenaltyCondition(BaseModel):
    ...
    penalty_type: str = Field("liquidated_damages", description="Type: liquidated_damages, service_credit, termination_right")
    amount: Optional[float] = None
    amount_formula: Optional[str] = Field(None, description="Formula for computing penalty, e.g. '5% of monthly fee per day'")
    cap: Optional[float] = Field(None, description="Maximum penalty cap amount")
...
class BillableEvent(BaseModel):
    ...
    rate_ref: Optional[str] = Field(None, description="Reference to the applicable rate card entry")
    requires_approval: bool = Field(False, description="Whether pre-approval is needed before billing")
    excluded_activities: list[str] = Field(default_factory=list, description="Activities explicitly excluded from billing")
    linked_clause_ids: list[str] = Field(default_factory=list)
...
class BillabilityDecision(BaseModel):
    ...
    evidence_ids: list[str] = Field(default_factory=list, description="IDs of clauses/events that support this decision")
    reasons: list[str] = Field(default_factory=list)
    rule_results: list[RuleResult] = Field(default_factory=list)
    category: Optional[BillableCategory] = None
...
class MarginLeakageDiagnosis(BaseModel):
    ...
    total_estimated_leakage: Optional[float] = None
    currency: str = Field("USD")
...
class PenaltyExposureSummary(BaseModel):
    ...
    total_exposure_amount: Optional[float] = None
    currency: str = Field("USD")
    ...
    highest_risk_penalty: Optional[PenaltyCondition] = None
