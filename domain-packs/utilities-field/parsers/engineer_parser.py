...
_SKILL_KEYWORDS: dict[SkillCategory, re.Pattern[str]] = {
    SkillCategory.electrical: re.compile(
        r"\b(electric|wiring|circuit|power|18th\s+edition)\b", re.I
    ),
    SkillCategory.plumbing: re.compile(r"\b(plumb|pipe|water|drain|unvented)\b", re.I),
    SkillCategory.hvac: re.compile(
        r"\b(hvac|heating|ventilation|air\s+condition|f-gas|refrigerant)\b", re.I
    ),
...
_PROFICIENCY_KEYWORDS = {
    ...
    "competent": re.compile(
        r"\b(competent|qualified|experienced|mid[- ]?level|5\+?\s+years?)\b", re.I
    ),
...
_ACCREDITATION_PATTERNS: list[tuple[str, str, re.Pattern[str]]] = [
    (
        "Gas Safe Register",
        "Gas Safe Register",
        re.compile(r"gas\s+safe\s+(?:register|registered|reg)", re.I),
    ),
    ...
    (
        "Confined Space",
        "City & Guilds",
        re.compile(r"confined\s+space\s+(?:cert|trained|qualified)", re.I),
    ),
    (
        "F-Gas Certificate",
        "City & Guilds",
        re.compile(r"f[- ]?gas\s+(?:cert|qualified|registered)", re.I),
    ),
...
_CERT_NUMBER_PATTERN = re.compile(
    r"(?:cert(?:ificate)?|reg(?:istration)?|licence|card)\s*(?:#|no|number)?\s*[:=]?\s*([\w\-/]+)",
    re.I,
)

_FIELD_PATTERNS = {
    "name": re.compile(r"(?:name|engineer)\s*[:=]\s*(.+?)(?:\n|$)", re.I),
    "employee_number": re.compile(
        r"(?:employee|staff|badge|id)\s*(?:number|no|#)?\s*[:=]\s*([\w\-]+)", re.I
    ),
...
class EngineerProfileParser:
    ...
    def parse_profile(self, text: str) -> EngineerProfile:
        ...
        available = not bool(
            re.search(r"\b(unavailable|on\s+leave|off\s+sick|absent)\b", text, re.I)
        )

        # Check for current assignment
        assignment_match = re.search(
            r"(?:current|assigned\s+to|working\s+on)\s*[:=]?\s*([\w\-]+)", text, re.I
        )
        ...
        for name, issuer, pattern in _ACCREDITATION_PATTERNS:
            if pattern.search(text):
                ...
                if match:
                    ...
                    is_mandatory = bool(
                        re.search(r"\b(mandatory|legally\s+required|statutory)\b", context, re.I)
                    )
