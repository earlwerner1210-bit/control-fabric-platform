...
class Accreditation(BaseModel):
    ...
    categories: list[SkillCategory] = Field(
        default_factory=list, description="Skill categories this accreditation covers"
    )
    is_mandatory: bool = Field(
        False, description="Whether this is a legally required accreditation"
    )
...
class PermitRequirement(BaseModel):
    ...
    conditions: list[str] = Field(
        default_factory=list, description="Special conditions attached to the permit"
    )
...
class FieldJob(BaseModel):
    ...
    safety_equipment: list[str] = Field(
        default_factory=list, description="Required PPE and safety equipment"
    )
...
class SkillFitAnalysis(BaseModel):
    ...
    partially_matched: list[str] = Field(
        default_factory=list, description="Skills where engineer has related but not exact match"
    )
...
class ComplianceBlocker(BaseModel):
    ...
    category: str = Field(
        ..., description="Category: permit, accreditation, safety, access, schedule"
    )
    description: str
...
class DispatchRecommendation(BaseModel):
    ...
    work_order_id: str
    recommended_engineer_id: str | None = None
    readiness: ReadinessDecision
    alternative_engineers: list[str] = Field(
        default_factory=list, description="IDs of alternative engineers if primary is not suitable"
    )
    estimated_travel_time_minutes: float | None = None
