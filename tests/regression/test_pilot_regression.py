            ...
            EvidenceBundleCreate(
                ...
                items=[
                    EvidenceItem(
                        ...
                        source_label="MSA Contract v3.2",
                    ),
                    EvidenceItem(
                        evidence_type="document",
                        source_id=uuid.uuid4(),
                        source_label="Rate Card 2024-Q1",
                    ),
                    EvidenceItem(evidence_type="chunk", source_id=uuid.uuid4(), confidence=0.94),
                    EvidenceItem(
                        evidence_type="control_object",
                        source_id=uuid.uuid4(),
                        source_label="Scope Line Item",
                    ),
                ],
                chain_stages=[
                    "contract_basis",
...
class TestClosedCaseCannotTransition:
    ...
    def test_closed_is_terminal(self, platform):
        p = platform
        case = p["case"].create_case(
            TENANT, PilotCaseCreate(title="Close Test", workflow_type="test"), ANALYST
        )
...
class TestTenantIsolation:
    ...
    def test_cases_isolated_by_tenant(self, platform):
        ...
        p["case"].create_case(
            tenant_a, PilotCaseCreate(title="T-A-2", workflow_type="test"), ANALYST
        )
