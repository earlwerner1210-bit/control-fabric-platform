...
class EscalationRuleEngine:
    ...
    def evaluate(
        ...
        if sla_breached:
            should_escalate = True
            level = (
                EscalationLevel.management if level == EscalationLevel.l3 else EscalationLevel.l3
            )
        ...
        if len(affected_critical) >= 2:
            ...
            reason = (
                f"Multiple critical services affected: {', '.join(affected_critical)}. {reason}"
            )
...
class ReconciliationRuleEngine:
    ...
    def reconcile_incident_and_service_state(
        ...
        if incident.state == IncidentState.closed and service_state.state in (
            ServiceState.outage,
...
class DispatchNeedEngine:
    ...
    def determine_dispatch_need(
        ...
        remote_exhausted = (
            has_remote_resolution is False and incident.state == IncidentState.investigating
        )
        ...
        customer_requested = (
            "onsite" in text or "on-site" in text or "customer requested" in text.lower()
        )
        ...
        sla_requires = False
        if "sla" in " ".join(incident.tags).lower() and incident.severity in (
            IncidentSeverity.p1,
            IncidentSeverity.p2,
        ):
            sla_requires = True
...
class VodafoneSLAEngine:
    ...
    def check_sla_status(
        ...
        # Response SLA
        response_status = self._threshold_status(current_time_minutes, sla.response_time_minutes)
        ...
        if sla.update_frequency_minutes > 0 and current_time_minutes > 0:
            update_overdue = (
                current_time_minutes % sla.update_frequency_minutes
            ) == 0 or current_time_minutes > sla.update_frequency_minutes
...
class VodafoneEscalationEngine:
    ...
    def evaluate(
        ...
        is_outage = any(
            s in ("outage",) for s in [t.lower() for t in incident.tags]
        ...
        if repeat_count >= 3:
            should_escalate = True
            if level is None or self._LEVEL_ORDER.get(level.value, 0) < self._LEVEL_ORDER.get(
                "l3", 3
            ):
                level = EscalationLevel.l3
            reasons.append(
                f"Repeated incident ({repeat_count} occurrences in 30 days): L3 escalation"
            )
    ...
    def validate_closure(
        ...
        results.append(
            RuleResult(
                ...
                if restored
                else "Service has not been confirmed as restored",
        ...
        results.append(
            RuleResult(
                ...
                if notified
                else "Customer has not been notified of resolution",
        ...
        unsatisfied_mandatory = [g for g in closure_gates if g.mandatory and not g.satisfied]
