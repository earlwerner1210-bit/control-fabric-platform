...
def full_work_order() -> ParsedWorkOrder:
    return ParsedWorkOrder(
        ...
        required_skills=[
            ...
            SkillRecord(
                skill_name="electrical", category=SkillCategory.electrical, level="qualified"
            ),
        ],
        required_permits=[
            PermitRequirement(
...
class TestCompileFullWorkOrder:
    ...
    def test_compile_returns_result(
        ...
        full_work_order: ParsedWorkOrder,
        full_engineer: EngineerProfile,
    ):
    ...
    def test_dispatch_preconditions_generated(
        ...
        permit_preconds = [
            p
            for p in result.dispatch_preconditions
    ...
    def test_skill_requirements_generated(
        ...
        skill_names = {
            s["skill_name"]
            for s in result.skill_requirements
            if s.get("control_type") == "skill_requirement"
        }
...
class TestCompileSafetyPreconditions:
    ...
    def test_repair_gets_risk_assessment(
        self, compiler: FieldCompiler, full_work_order: ParsedWorkOrder
    ):
    ...
    def test_all_work_orders_get_ppe(
        self, compiler: FieldCompiler, minimal_work_order: ParsedWorkOrder
    ):
    ...
    def test_confined_space_permit_adds_preconditions(
        self, compiler: FieldCompiler, full_work_order: ParsedWorkOrder
    ):
        preconditions = compiler.compile_safety_preconditions(full_work_order)
        types = {p["precondition_type"] for p in preconditions}
    ...
    def test_gas_skill_adds_certification(self, compiler: FieldCompiler):
        wo = ParsedWorkOrder(
            ...
            required_skills=[
                SkillRecord(
                    skill_name="gas_fitting", category=SkillCategory.gas, level="qualified"
                ),
            ],
        )
        preconditions = compiler.compile_safety_preconditions(wo)
    ...
    def test_precondition_fields_are_populated(
        self, compiler: FieldCompiler, full_work_order: ParsedWorkOrder
    ):
        preconditions = compiler.compile_safety_preconditions(full_work_order)
    ...
    def test_electrical_skill_adds_ppe(
        self, compiler: FieldCompiler, full_work_order: ParsedWorkOrder
    ):
        preconditions = compiler.compile_safety_preconditions(full_work_order)
        elec_ppe = [
...
class TestCompileSkillRequirements:
    ...
    def test_specialist_skills_flagged(
        self, compiler: FieldCompiler, full_work_order: ParsedWorkOrder
    ):
        skills = compiler.compile_skill_requirements(full_work_order)
    ...
    def test_implied_skills_from_permits(
        self, compiler: FieldCompiler, full_work_order: ParsedWorkOrder
    ):
        skills = compiler.compile_skill_requirements(full_work_order)
...
class TestCompilePermitRequirements:
    ...
    def test_permit_preconditions_count(
        self, compiler: FieldCompiler, full_work_order: ParsedWorkOrder
    ):
    ...
    def test_unobtained_permit_is_blocking(
        self, compiler: FieldCompiler, full_work_order: ParsedWorkOrder
    ):
        preconds = compiler.compile_dispatch_preconditions(full_work_order, [])
    ...
    def test_obtained_permit_not_blocking(
        ...
        building = [
            ...
            for p in preconds
            if p.get("control_type") == "permit_precondition"
            and p.get("permit_type") == "building_access"
        ]
...
class TestCompileMaterials:
    ...
    def test_materials_availability_check(
        self, compiler: FieldCompiler, full_work_order: ParsedWorkOrder
    ):
        preconds = compiler.compile_dispatch_preconditions(full_work_order, [])
...
class TestCompileMinimalData:
    ...
    def test_minimal_compile_succeeds(
        self,
        compiler: FieldCompiler,
        minimal_work_order: ParsedWorkOrder,
        minimal_engineer: EngineerProfile,
    ):
