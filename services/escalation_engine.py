    ...
    def _check_p1(self, priority: str, level: int) -> EscalationAction | None:
        if priority == "P1" and level < 2:
            return EscalationAction(
                "auto_escalate_to_senior_engineer",
                2,
                "P1 incident requires automatic escalation",
                "critical",
            )
        return None
            ...
            if level < 3:
                actions.append(
                    EscalationAction(
                        ...
                        3,
                        f"SLA breached ({elapsed:.0f}/{res_target} min)",
                        "critical",
                    )
                )
        elif pct >= 0.75:
    ...
    def _check_vip(self, incident: dict, level: int) -> EscalationAction | None:
        ...
        if total > 100 and level < 3:
            return EscalationAction(
                ...
                3,
                f"VIP customer with {total} affected services",
                "critical",
            )
        return None
    ...
    def _check_unack(self, incident: dict, targets: dict, now: datetime, level: int):
        ...
        if remaining <= 30 and level < 1:
            return EscalationAction(
                ...
                1,
                f"Response deadline within {remaining:.0f} min, unacknowledged",
                "urgent",
            )
        return None
